...
6. **Check for existing code.** Search the project for existing dbt models for this data product. If models exist, read them, diff against the current contract and quality config, and propose updates to align. If no models exist, proceed with generation.
   ...
   **Composite key detection:** If any existing model uses a list for `unique_key` (e.g., `unique_key=['station', 'date', 'data_type']`), flag it and recommend a surrogate key. See [dbt-model-patterns.md](references/dbt-model-patterns.md) for the surrogate key pattern.
...
Generate one staging model per source table. Refer to [dbt-model-patterns.md](references/dbt-model-patterns.md) for layer conventions, naming patterns, and the surrogate key pattern for composite keys.
...
**Composite key handling:** If the contract or source schema defines a composite primary key (multiple columns), recommend generating a surrogate `id` column via `dbt_utils.generate_surrogate_key`. Switch `unique_key` to `'id'` for a cleaner merge key. Keep the composite uniqueness test as validation. See [dbt-model-patterns.md](references/dbt-model-patterns.md) for the pattern and example.
